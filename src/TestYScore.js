...
const INIT_SCORE_DISPLAY = Array(15).fill("");

const INIT_CANBEPLAYED =
[
...
const INIT_SCORE = Array(13).fill(0);
...
class YRoll extends YArray{

  NROUNDS = 3;  

  constructor(arr,holdmask){
    const input = arr || [0,0,0,0,0];
    const inputhold = holdmask || [false,false,false,false,false]
    super(...input);
  ...
  *[Symbol.iterator]() {
  ...
  get scoredisplay () {
    return [
            ...this._score.slice(0, BONUS_ROW), // top
            "", // Bonus row
            ...this._score.slice(BONUS_ROW), // bottom
            ""  // Total row
            ];
  };
...
class YScore {
    ...
    **/
    ...
    Import(json){
      ...
      this._overallscore = obj.overallscore;
      this._canbeplayed = obj.canbeplayed;
    }
...
class YGame {
  constructor(gameid, lobby, gametype, islocalgame, currentplayerindex){
    this.gameid = gameid;
    this.lobby = lobby || [];
  ...
  PassTurn (){
    ...
    if (this.lobby.every(p => p.yscore.RoundsLeftToPlay()===0 )) 
    {
...
lobby.AddPlayer("Teo",2,"pros");
lobby.AddPlayer("dummy", 3, "totos");
lobby.SetGameMode (GAMETYPE.doublebonus, false);
lobby.RemovePlayer(lobby.GetIndexOfPlayerByName("dummy"))

let game = lobby.StartGame();

console.log(game.lobby[0].yscore);
game.Roll();

game.UpdateHoldMask([0,0,0,1,1]);
game.Roll();
game.ValidateScore(5);
